...
/** Generic simple middleware that always adds the domain */
...
export function createCorsOptionsMiddleware(
  options: CorsOptions
): RequestMiddleware {
  return async function test(
    ...
    const methodAllowed =
      options.allowMethods?.includes(method) || method === "OPTIONS";

    if (match && methodAllowed) {
      if (options.allowMethods) {
        // const s = new Set([...options.allowMethods, 'OPTIONS'])
